  ...
  card.innerHTML = `
    <div class="tooltip-wrapper">
      ...
      <span class="tag tag--match" style="position:absolute;top:8px;right:8px;">
        ${recommendation.matchScore || 0}% Match
      </span>
    </div>
    <div class="card__body">
      <div class="card__title" title="${recommendation.name || ''}">${recommendation.name || 'Unknown'}</div>
      <div class="card__meta">${genres}</div>
      <div class="score-bar">
        <div class="score-bar__fill" style="width:${recommendation.matchScore || 0}%"></div>
      </div>
      <div class="recommend-card__reason">${recommendation.reason || ''}</div>
      <div class="recommend-card__actions">
        <button class="btn btn--sm btn--primary" data-action="played" data-id="${recommendation.appId}">Played It</button>
        <button class="btn btn--sm btn--secondary" data-action="skip" data-id="${recommendation.appId}">Skip</button>
      </div>
    </div>
  `;
...
export function renderRecommendations(container, recommendations, emptyMessage) {
  if (!container) return;

  const recs = recommendations || [];
  const msg = emptyMessage || 'No recommendations yet';
  ...
  recs.forEach(rec => grid.appendChild(createRecommendCard(rec)));
